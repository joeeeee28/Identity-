...
import type { DashboardOverview, SearchResult, SessionUser } from './types'
import { Administration, Agents, AIWorkspace, AIEvaluation, Analytics, Automations, CommandCenter, Governance, History, Knowledge, KnowledgeIntelligence, Meetings, OperatingIntelligence, ProductHealth, ProductLearning, ReadinessCenter, Settings, ValueIntelligence } from './views'

type ViewKey = 'command-center' | 'ai-workspace' | 'knowledge' | 'meetings' | 'ai-agents' | 'automations' | 'knowledge-intelligence' | 'operating-intelligence' | 'analytics' | 'ai-evaluation' | 'product-health' | 'product-learning' | 'value-intelligence' | 'history' | 'governance' | 'readiness' | 'administration' | 'settings'

const navGroups: Array<{ label: string; items: Array<{ id: ViewKey; label: string; icon: string; badge?: string }> }> = [
  { label: 'Workspace', items: [{ id: 'command-center', label: 'Command center', icon: 'dashboard' }, { id: 'ai-workspace', label: 'AI workspace', icon: 'sparkles', badge: 'AI' }, { id: 'knowledge', label: 'Knowledge', icon: 'book' }, { id: 'meetings', label: 'Meetings', icon: 'calendar' }] },
...
export default function App() {
  ...
  const openSearchResult = (result: SearchResult) => { const target: Record<SearchResult['kind'], ViewKey> = { document: 'knowledge', meeting: 'meetings', agent: 'ai-agents', workflow: 'automations', audit: 'history' }; setGlobalSearch(''); setSearchResults([]); navigate(target[result.kind]) }
